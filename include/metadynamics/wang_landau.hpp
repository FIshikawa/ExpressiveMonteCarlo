...
public:
  ...
  WangLandau(unsigned int dim, unsigned int N_bin, double E_max, double E_min) : 
    dim_(dim), E_min_(E_min), E_max_(E_max), N_bins_(N_bin), entropy_(N_bin,0.0),
    density_(N_bin,0.0),bins_(N_bin,0.0)
    {bin_ = (E_max_ - E_min_) / (double)N_bins_;}

  template <typename Func, typename Proposer>
  void montecalro(Func const& f,  Proposer const& proposer, 
      const int max_iteration, std::mt19937 & mt, int mc_check = 0){
